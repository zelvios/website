<script lang="ts">
    ...
    import Explosion from "$lib/components/ui/ExplosionEffect.svelte";
...
<Motion animate={{
    ...
    <div bind:this={beamRef}
         class={`absolute left-0 top-20 m-auto h-14 w-px rounded-full bg-gradient-to-t from-accent2 via-accent to-transparent ${beam.class}`}
         use:motion></div>
</Motion>
{#if collision.detected && collision.coordinates}
    <Explosion
